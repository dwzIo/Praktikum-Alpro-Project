...
void saveDatabaseBuku() {
    ...
    for (int i = 0; i < MAX_RAK; i++) {
        for (int j = 0; j < MAX_SLOT; j++) {
            ...
            file << i << " " << j << " " << replaceSpacetoUnderscore(rakBuku[i][j].namaBuku)
                 << " " << replaceSpacetoUnderscore(rakBuku[i][j].namaPenulis)
                 << " " << rakBuku[i][j].tahunTerbit << " " << replaceSpacetoUnderscore(rakBuku[i][j].kategori)
                 << " " << rakBuku[i][j].harga << " "<< rakBuku[i][j].jumlahBest << " " << rakBuku[i][j].jumlahStok
...
void beliBuku(float diskon) {
    ...
    if (rakBuku[r][s].isTersedia && rakBuku[r][s].jumlahStok > 0) {
        cout << "\nBuku Terpilih:\n"
             << "Nama Buku      : " << rakBuku[r][s].namaBuku << "\n" 
             << "Harga (perbuku): " << rakBuku[r][s].harga - (rakBuku[r][s].harga * diskon) << "\n"
             << "Stok Tersedia  : " << rakBuku[r][s].jumlahStok << " pcs\n";
        do {
            ...
            if (jmlBeli <= 0 || jmlBeli > rakBuku[r][s].jumlahStok) {
        ...
        rakBuku[r][s].jumlahStok -= jmlBeli;
        if (rakBuku[r][s].jumlahStok == 0){
            rakBuku[r][s].isTersedia = false;
        } 
        rakBuku[r][s].jumlahBest += jmlBeli;
        p.namaBukuDibeli = rakBuku[r][s].namaBuku; 
...
void tambahbuku(int rak, int slot) {
    ...
    rakBuku[rak][slot].namaBuku = buku;
    ...
    cout << "Kategori (Fiksi/Non-Fiksi): "; getline(cin, rakBuku[rak][slot].kategori);
    do{
        sub = 0;
        cout << "Harga: "; cin >> temp;
        cin.ignore(1000, '\n');
...
void tambahstok(int rak, int slot) {
    string tam;
    int tamAngka;
    cout << "Nama Buku: " << rakBuku[rak][slot].namaBuku << "\n";
...
void hapusBuku(int rak, int slot) {
    for (int i = rak; i < MAX_RAK; i++) {
        for (int j = slot; j < MAX_SLOT - 1; j++) {
            if (rakBuku[i][j].isTersedia) {
                rakBuku[i][j] = rakBuku[i][j + 1];
            }
        }
    }
    saveDatabaseBuku();
...
void halamanAdmin() {
    ...
    do{
        ...
        if (pilAngka == 2) {
            ...
            cout << "Nama Buku: " << rakBuku[r][s].namaBuku << "\n";
            hapusBuku(r, s);
        }
        aksiSelesai();
    }while(pilAngka != 7);
